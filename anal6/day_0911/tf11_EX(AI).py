...
plt.scatter(feature.values.ravel(), label.values.ravel(), color='red', marker='o', label='real data')

# 예측 데이터 (x값 기준 정렬)
sorted_idx = np.argsort(feature_test.values.ravel())
plt.plot(feature_test.values.ravel()[sorted_idx], pred.ravel()[sorted_idx], 'b--', label='predicted data')
