...
# Ajax 및 일반 요청 모두 처리하는 index 함수 추가
from django.http import JsonResponse

@csrf_exempt  # CSRF 검증을 생략하여 Ajax 요청도 처리 가능하게 함
def index(request):
    # Ajax로 POST 요청이 오면 연봉 예측 결과를 반환
    if request.method == 'POST' and request.headers.get('x-requested-with') == 'XMLHttpRequest':
        years = float(request.POST.get('years', 0))  # 폼에서 입력받은 근무년수 추출
        model_path = 'c:/Users/htw02/HDRT_AI/PART2/myproject/LinearRegression_model.pkl'  # 저장된 모델 경로
        model = joblib.load(model_path)  # 모델 파일 로드
        predicted_salary = model.predict(np.array([[years]]))[0]  # 입력값으로 연봉 예측
        return JsonResponse({'predicted_salary': int(predicted_salary)})  # 예측 결과를 JSON으로 반환
    # 일반 GET 요청이면 index.html 렌더링
...
def linear_model_generation(request):
    ...
    train, test = train_test_split(data, test_size=0.5, random_state=1)
